...
class handle_aps:
    ...
    def __init__(self, data: np.ndarray or None):
        ...
        Inputs:
            ...
            shape (int, int): The shape of the full pixel array in the form of (height, width). Note: y-dimension first!
        Attributes:
    ...
    def load_file(self, file: str):
        """Load (and update) data from a NPY or AEDAT4 file.
        Args:
        ...
        if extension == 'aedat4':
            _, self.data, _ = load_aedat4_to_npy(file, undistorted=False)
        elif extension == 'npy':
    ...
    # ---------------------------------- Undistort data ------------------------------------
    def load_xml_calibration(self, file, sensor_model='DAVIS346', serial='00000336'):
    ...
    def _map_undistortion(self, shape=(260, 346)) -> (np.ndarray, np.ndarray):
    ...
    def _remap(self, map1: np.ndarray, map2: np.ndarray, interpolate=False):
    ...
    def undistort(self, shape=(260, 346), interpolate=False):
        ...
        Args:
             interpolate (bool): Whether to smoothen the result through interpolation.
        """
    ...
    def crop(self, shape: (int, int)):
        ...
        Args:
            shape ((int, int), required): number of pixels along the y and x dimensions of the new pixel array.
        """
    ...
    def crop_region(self, start: tuple, end: tuple):
    ...
    def cut_duration(self, duration: int, refresh=40):
        ...
        Args:
            timestamps (np.ndarray, required): timestamps of the frames in the apsnpy array (must be 1D-array with shape N).
            duration (int, required): wanted duration (in us) for the recorded frames.
        Returns:
            aps_cut (np.ndarray): frames array as apsnpy but without frames generated after the desired duration.
        """
        dt = 10 ** 6 / refresh
    ...
    # ------------------------------------- Show frames ------------------------------------
    def show_frame(self, num_frame=0, title=None, show=True, figsize=(5, 4)):
        """Show a single frame.
        Args:
            num_frame (int): the frame to show.
    ...
    def show_video(self, refresh=40, title='APS video', position=(0, 0), zoom=1):
...
class handle_dvs:
    ...
    def __init__(self, data: np.ndarray or None, shape=(260, 346)):
        """
    ...
    # ------------------------------------ Update data -------------------------------------
    def load_file(self, file: str, shape=(260, 346)):
        """Load (and update) data from a NPY or AEDAT4 file.
        Args:
            file (str): full path of the file where events were recorded.
        ...
        extension = file.split('.')[-1]
        if extension == 'aedat4':
            data, _, _ = load_aedat4_to_npy(file, undistorted=False)
            self.data = self.timereset(data)
        elif extension == 'npy':
    ...
    def load_array(self, data: np.ndarray, shape=(260, 346)):
        """Load (and update) data from a numpy Nd-array.
        Args:
            data (np.ndarray): Events in the form of a Nx4 np.ndarray where N is the total number of events and the
                columns specify the timestamp, the x and y pixel address, and the polarity of all such events.
    ...
    def _inverse_map_undistortion(self, shape=(260, 346)) -> (np.ndarray, np.ndarray):
    ...
    def undistort(self, shape=(260, 346)):
    ...
    # ------------------------------------ Filter data -------------------------------------
    def refractory_filter(self, refractory=100, return_filtered=False, verbose=False):
        ...
        Args:
            ...
            return_filtered (bool): if True, the indices of filtered events are returned.
        """
        t0 = time.time()
        address, timestamp = self.id, self.ts
    ...
    def denoising_filter(self, time_window=1e4, space_window=3):
        ...
        Args:
            time_window (int): The maximum temporal distance (in micro-seconds) to the next event, otherwise it is
                dropped. Lower values will therefore mean more events will be filtered out.
        """
    ...
    def instantaneous_firingrate(self, timestamps=None, num_neurons=0, duration=0, window=50000, smooth=True) -> np.ndarray:
    ...
    def show_IFR(self, ifr: np.ndarray, duration=None, show=True, figsize=None):
        if not duration:
            duration = self.duration
    ...
    def spike_train(self, neuron_id: tuple or int, return_id=False) -> (np.ndarray, np.ndarray) or (np.ndarray, np.ndarray, np.ndarray):
    ...
    def show_spiketrain(self, timestamps: np.ndarray or list, polarities: np.ndarray or list, duration=None,
                        title='Spike Train', show=True, figsize=(12, 2)):
        if not duration:
            duration = self.duration
        pol_on = (polarities == 1)
        pol_off = np.logical_not(pol_on)
    ...
    def show_rasterplot(self, data=None, show=True, figsize=None):
        if data is None:
    ...
    def timesurface(self, data=None, dt=None, duration=0, verbose=False) -> np.ndarray:
    ...
    def timesurface_onoff(self, dt=None, duration=0) -> (np.ndarray, np.ndarray):
    ...
    def show_timesurface(self, duration=0, fullscreen=False, show=True, figsize=None):
    ...
    def show_timesurface_onoff(self, duration=0, fullscreen=False, show=True, figsize=None):
    ...
    # --------------------------------- Reconstruct frames ---------------------------------
    def frame(self, data=None, clip_value=0.5) -> np.ndarray:
    ...
    def show_frame(self, clip_value=0.5, title="DVS events accumulator", show=True, figsize=(5, 4)):
    ...
    def frame_firingrate(self, data=None, duration=0) -> np.ndarray:
        ...
        Args:
            ...
            duration (float, optional): if it is specified, the entered value is used to compute the firing rate,
                otherwise the duration is computed from dvsnpy.
        Returns:
    ...
    def show_frame_firingrate(self, title="DVS events accumulator", show=True, figsize=(5, 4)):
    ...
    def show_frame_firingrate_onoff(self, title="On and Off DVS events accumulator", show=True, figsize=(5, 8)):
        """Show the single frame obtained accumulating all DVS events in time.
        """
    ...
    # --------------------------------- Reconstruct video ----------------------------------
    def video(self, refresh=50, clip_value=0.5) -> np.ndarray:
        """Given an array of events from the DVS, with N rows (number of events) and 4 columns (timestamps, x and y
    ...
    def show_video(self, refresh=50, clip_value=0.5, title="DVS reconstructed video", position=(0, 0), zoom=1):
    ...
    def video_firingrate(self, data=None, refresh=50, duration=0) -> np.ndarray:
        """Given an array of events from the DVS, with N rows (number of events) and 3 columns (timestamps, flatten
    ...
    def video_firingrate_onoff(self, refresh=50) -> (np.ndarray, np.ndarray):
        """Given an array of events from the DVS, with N rows (number of events) and 3 columns (timestamps, flatten
    ...
    def show_video_firingrate(self, refresh=50, title='DVS reconstructed video', position=(0, 0), zoom=1):
    ...
    def show_video_firingrate_onoff(self, refresh=50, title='On-Off DVS reconstructed video', position=(0, 0), zoom=1):
        """Show the video from an array of DVS-reconstructed frames (obtained accumulating events in a time-window).
        """
        dt = 0 if not refresh else int(round(10 ** 3 / refresh))
